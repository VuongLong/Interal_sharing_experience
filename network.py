...
class PGNetwork_deeper:
    ...
    def __init__(self, state_size, action_size, learning_rate, name='PGNetwork_deeper'):
        ...
        with tf.variable_scope(name):
            ...
            self.W_fc2 = self._fc_weight_variable([256, 256])
            self.b_fc2 = self._fc_bias_variable([256], 256)
            self.fc2 = tf.nn.relu(tf.matmul(self.fc1, self.W_fc2) + self.b_fc2)

            self.W_fc3 = self._fc_weight_variable([256, self.action_size])
            self.b_fc3 = self._fc_bias_variable([self.action_size], 256)
