...
class PGNetwork:
	...
	def __init__(self, state_size, task_size, action_size, learning_rate, name='PGNetwork'):
		...
		with tf.variable_scope(name):
			...
			'''
			self.W_fc1 = self._fc_weight_variable([self.state_size, self.action_size])
			self.b_fc1 = self._fc_bias_variable([self.action_size], self.state_size)
			self.logits = tf.matmul(self.inputs, self.W_fc1) + self.b_fc1
			self.variables = [self.W_fc1, self.b_fc1]
			...
			self.inter_train_opt = tf.train.RMSPropOptimizer(self.learning_rate).minimize(self.loss)
